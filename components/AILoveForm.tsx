...
export default function LoveLetterWriter() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.relationshipDuration.trim()) {
      newErrors.relationshipDuration = 'Relationship duration is required';
    }
    setErrors(newErrors);
    ...
    <div className="max-w-7xl mx-auto p-4 mt-10">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Love Letter Parameters</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.partnerName) === characterLimits.partnerName ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.partnerName}/{characterLimits.partnerName}
                  </p>
                </div>
                {errors.partnerName && (
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder="Enter the occasion (e.g., anniversary, Valentine's Day)..."
                    className={errors.occasion ? 'border-red-500' : ''}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.occasion) === characterLimits.occasion ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.occasion}/{characterLimits.occasion}
                  </p>
                </div>
                {errors.occasion && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
              ...
              <div className="space-y-2">
                <Label htmlFor="relationshipDuration" className="text-sm font-medium text-gray-700">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder="How long have you been together? (e.g., 2 years, 6 months)..."
                    className={errors.relationshipDuration ? 'border-red-500' : ''}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.relationshipDuration) === characterLimits.relationshipDuration ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.relationshipDuration}/{characterLimits.relationshipDuration}
                  </p>
                </div>
                {errors.relationshipDuration && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
                    <AlertCircle className="h-4 w-4 mr-1" />
                    {errors.relationshipDuration}
                  </p>
        ...
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Generated Love Letter</CardTitle>
          </CardHeader>
          <CardContent>
            {generatedLetter ? (
              <>
              ...
              <p className="text-gray-500 italic">Your generated love letter will appear here.</p>
            )}
          </CardContent>
        </Card>
      </div>
