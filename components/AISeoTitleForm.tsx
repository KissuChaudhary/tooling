  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/openai-api', {
        ...
        body: JSON.stringify({
          tool: 'aiSEOTitleGenerator',
          model,
          data: { ...formData, count: titleCount },
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(data.error || 'An error occurred while generating SEO titles');
      }

      setGeneratedTitles(data.seoTitle);
    } catch (error) {
    ...
    <div className="max-w-7xl mx-auto p-4 mt-10">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>SEO Title Parameters</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.keywords) === characterLimits.keywords ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.keywords}/{characterLimits.keywords}
                  </p>
                </div>
                {errors.keywords && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
              ...
              <div className="space-y-2">
                <Label htmlFor="titleCount" className="text-sm font-medium text-gray-700">
                  Number of Titles
                </Label>
                <Select onValueChange={(value) => setTitleCount(parseInt(value))} defaultValue={titleCount.toString()}>
                  <SelectTrigger id="titleCount">
                    <SelectValue placeholder="Select number of titles" />
                  </SelectTrigger>
                  <SelectContent>
                    {[1, 3, 5, 7, 10].map((num) => (
        ...
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Generated SEO Titles</CardTitle>
          </CardHeader>
          <CardContent>
            {generatedTitles.length > 0 ? (
              ...
              <p className="text-gray-500 italic">Your generated SEO titles will appear here.</p>
            )}
          </CardContent>
        </Card>
      </div>
