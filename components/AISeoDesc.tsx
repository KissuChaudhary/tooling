...
export default function SeoMetaDescriptionGenerator() {
  ...
  const handleChange = (name: keyof FormData, value: string, index?: number) => {
    if (name === 'title' && value.length <= characterLimits.title) {
      setFormData(prev => ({ ...prev, [name]: value }));
      setCharacterCounts(prev => ({ ...prev, [name]: value.length.toString() }));
    } else if (name === 'keywords' && typeof index === 'number' && value.length <= characterLimits.keyword) {
      const newKeywords = [...formData.keywords];
      newKeywords[index] = value;
      setFormData(prev => ({ ...prev, keywords: newKeywords }));
      setCharacterCounts(prev => ({
        ...prev,
        keywords: prev.keywords.map((count, i) => i === index ? value.length.toString() : count)
      }));
    }
    if (errors[name]) {
  ...
  const addKeyword = () => {
    ...
    setCharacterCounts(prev => ({ ...prev, keywords: [...prev.keywords, '0'] }));
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.keywords.every(keyword => !keyword.trim())) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/openai-api', {
        ...
        body: JSON.stringify({
          tool: 'aiSeoMetaDescriptionGenerator',
          model,
          data: formData,
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(data.error || 'An error occurred while generating the meta description');
      }

      setGeneratedDescription(data.metaDescription);
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 mt-10">
      <h1 className="text-4xl font-extrabold mb-8 text-center tracking-tight">AI SEO Meta Description Generator</h1>
      <p className="text-xl text-center mb-12 max-w-3xl mx-auto">Generate Optimized Meta Descriptions with Saze AI – Boost Your SEO Performance.</p>
      <AdUnit 
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.title) === characterLimits.title ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.title}/{characterLimits.title}
                  </p>
                </div>
                {errors.title && (
              ...
              <div className="space-y-2">
                ...
                {formData.keywords.map((keyword, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    <div className="relative flex-grow">
                      <Input
                        value={keyword}
                        onChange={(e) => handleChange('keywords', e.target.value, index)}
                        placeholder={`Keyword ${index + 1}`}
                        className={errors.keywords ? 'border-red-500' : ''}
                      />
                      <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.keywords[index]) === characterLimits.keyword ? 'text-orange-500' : 'text-gray-500'}`}>
                        {characterCounts.keywords[index]}/{characterLimits.keyword}
                      </p>
                    </div>
                    {index > 0 && (
                ...
                <Button type="button" variant="outline" onClick={addKeyword} className="w-full mt-2">
        ...
        <Card className="w-full md:w-1/2">
          ...
          <CardContent>
            {generatedDescription ? (
              ...
              <p className="text-gray-500 italic">Your generated meta description will appear here.</p>
            )}
          </CardContent>
        </Card>
      </div>
