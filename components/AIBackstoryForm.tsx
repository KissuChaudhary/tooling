...
export default function BackstoryGenerator() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.keyEvents.trim()) {
      newErrors.keyEvents = 'Key events are required';
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'An error occurred while generating the backstory');
      }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 mt-10">
      <h1 className="text-4xl font-extrabold mb-8 text-center tracking-tight">AI Backstory Generator</h1>
      <p className="text-xl text-center mb-12 max-w-3xl mx-auto">Create Compelling Character Backstories with Saze AI – Bring Your Characters to Life.</p>
      <AdUnit 
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Backstory Parameters</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="characterName" className="text-sm font-medium text-gray-700">
                  Character Name
                </Label>
                <div className="relative">
                  ...
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.characterName) === characterLimits.characterName ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.characterName}/{characterLimits.characterName}
                  </p>
                </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="keyEvents" className="text-sm font-medium text-gray-700">
                  Key Events
                </Label>
                <div className="relative">
                  <Textarea
                    ...
                    className={`resize-none h-32 ${errors.keyEvents ? 'border-red-500' : ''}`}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.keyEvents) === characterLimits.keyEvents ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.keyEvents}/{characterLimits.keyEvents}
                  </p>
                </div>
                {errors.keyEvents && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
            ...
            {generatedBackstory ? (
              <>
                <div className="bg-gray-100 p-4 rounded-md mb-4 max-h-96 overflow-y-auto">
                  <p className="whitespace-pre-wrap">{generatedBackstory}</p>
                </div>
              ...
              <p className="text-gray-500 italic">Your generated character backstory will appear here.</p>
            )}
          </CardContent>
        </Card>
      </div>
