...
export default function CharacterGenerator() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.traits.trim()) {
      newErrors.traits = 'Traits are required';
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'An error occurred while generating the character');
      }
    ...
    <div className="max-w-7xl mx-auto p-4 mt-10">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Character Parameters</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder="Enter the genre (e.g., fantasy, sci-fi, mystery)..."
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder="Enter the character's role (e.g., protagonist, villain, sidekick)..."
                    className={errors.role ? 'border-red-500' : ''}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.role) === characterLimits.role ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.role}/{characterLimits.role}
                  </p>
                </div>
                {errors.role && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Textarea
                    ...
                    placeholder="Enter character traits (e.g., brave, intelligent, mysterious)..."
                    className={`resize-none ${errors.traits ? 'border-red-500' : ''}`}
                    rows={3}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.traits) === characterLimits.traits ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.traits}/{characterLimits.traits}
                  </p>
                </div>
                {errors.traits && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
            ...
            {generatedCharacter ? (
              <>
                <div className="bg-gray-100 p-4 rounded-md mb-4 max-h-96 overflow-y-auto">
                  <p className="whitespace-pre-wrap">{generatedCharacter}</p>
                </div>
              ...
              <p className="text-gray-500 italic">Your generated character description will appear here.</p>
            )}
          </CardContent>
        </Card>
      </div>
