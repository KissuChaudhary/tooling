...
export default function EmailResponseGenerator() {
  ...
  const handleInputChange = (name: 'originalEmail' | 'response', value: string) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.originalEmail.trim()) {
      newErrors.originalEmail = 'Original email is required';
    }
    if (!formData.response.trim()) {
      newErrors.response = 'Response points are required';
    }
    setErrors(newErrors);
    ...
    <div className="max-w-7xl mx-auto p-4 mt-10">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Email Response Parameters</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="originalEmail" className="text-sm font-medium text-gray-700">
                  Original Email
                </Label>
                <div className="relative">
                  <Textarea
                    ...
                    className={`resize-none h-40 ${errors.originalEmail ? 'border-red-500' : ''}`}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.originalEmail) === characterLimits.originalEmail ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.originalEmail}/{characterLimits.originalEmail}
                  </p>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Textarea
                    ...
                    placeholder="Enter key points for your response..."
                    className={`resize-none h-32 ${errors.response ? 'border-red-500' : ''}`}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.response) === characterLimits.response ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.response}/{characterLimits.response}
                  </p>
                </div>
                {errors.response && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
              ...
              <div className="space-y-2">
                ...
                <Select onValueChange={handleToneChange} defaultValue={formData.tone}>
        ...
        <Card className="w-full md:w-1/2">
          ...
          <CardContent>
            {generatedResponse ? (
              ...
              <p className="text-gray-500 italic">Your generated email response will appear here.</p>
            )}
          </CardContent>
        </Card>
      </div>
