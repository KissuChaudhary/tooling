...
const fieldOfStudyOptions = [
  'Literature', 'History', 'Psychology', 'Sociology', 'Political Science',
  'Economics', 'Biology', 'Chemistry', 'Physics', 'Computer Science',
  'Environmental Science', 'Philosophy', 'Art History', 'Education',
  'Engineering', 'Mathematics', 'Linguistics', 'Anthropology', 'Business',
  'Law'
];
...
export default function AIThesisStatementGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/openai-api', {
        ...
        body: JSON.stringify({
          tool: 'thesisStatementGenerator',
          model,
          data: formData,
        }),
      });
      if (!response.ok) {
        throw new Error('Failed to generate thesis statement');
      }
      const data = await response.json();
      setGeneratedThesis(data.thesisStatement);
    } catch (error) {
      console.error('Error:', error);
      setErrors({ submit: 'Failed to generate thesis statement. Please try again.' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 mt-10">
      <h1 className="text-4xl font-extrabold mb-8 text-center tracking-tight">AI Thesis Statement Generator</h1>
      <p className="text-xl text-center mb-12 max-w-3xl mx-auto">Craft Compelling Thesis Statements with Saze AI – Perfect for Academic Writing and Research Papers!</p>
      <AdUnit 
...
/>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Thesis Statement Details</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <div key={field} className="relative">
                  <div className="flex justify-between items-center mb-1">
                    <Label htmlFor={field} className="text-sm font-medium text-gray-400 capitalize">
                      {field === 'fieldOfStudy' ? 'Field of Study' : field}
                    </Label>
        ...
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Generated Thesis Statement</CardTitle>
          </CardHeader>
          <CardContent>
            {generatedThesis ? (
              <>
                <div className="bg-gray-100 p-4 rounded-md mb-4">
                  <p className="whitespace-pre-wrap">{generatedThesis}</p>
                </div>
              ...
              <p className="text-gray-500 italic">Your generated thesis statement will appear here.</p>
            )}
          </CardContent>
        </Card>
      </div>
