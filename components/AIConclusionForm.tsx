...
export default function ConclusionGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'An error occurred while generating the conclusion');
      }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 mt-10">
      <h1 className="text-4xl font-extrabold mb-8 text-center tracking-tight">AI Conclusion Generator</h1>
      <p className="text-xl text-center mb-12 max-w-3xl mx-auto">Create Powerful Conclusions with Saze AI – Summarize Your Ideas Effectively.</p>
      <AdUnit 
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Conclusion Parameters</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder="Enter the main topic of your conclusion..."
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Textarea
                    ...
                    placeholder="Enter the key points to be summarized in the conclusion..."
                    className={`resize-none ${errors.keyPoints ? 'border-red-500' : ''}`}
                    rows={4}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.keyPoints) === characterLimits.keyPoints ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.keyPoints}/{characterLimits.keyPoints}
                  </p>
                </div>
                {errors.keyPoints && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder="Enter the desired tone (e.g., formal, persuasive, reflective)..."
                    className={errors.tone ? 'border-red-500' : ''}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.tone) === characterLimits.tone ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.tone}/{characterLimits.tone}
                  </p>
                </div>
                {errors.tone && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
        ...
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Generated Conclusion</CardTitle>
          </CardHeader>
          <CardContent>
            {generatedConclusion ? (
              <>
                <div className="bg-gray-100 p-4 rounded-md mb-4 max-h-96 overflow-y-auto">
                  <p className="whitespace-pre-wrap">{generatedConclusion}</p>
                </div>
              ...
              <p className="text-gray-500 italic">Your generated conclusion will appear here.</p>
            )}
          </CardContent>
        </Card>
      </div>
