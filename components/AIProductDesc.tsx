  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/openai-api', {
        ...
        body: JSON.stringify({
          tool: 'aiProductDescriptionGenerator',
          model,
          data: formData,
        }),
      });
...
/>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <Card className="w-full md:w-1/2">
          <CardHeader>
            <CardTitle>Product Description Parameters</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.productName) === characterLimits.productName ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.productName}/{characterLimits.productName}
                  </p>
                </div>
                {errors.productName && (
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Textarea
                    ...
                    className={`resize-none h-32 ${errors.features ? 'border-red-500' : ''}`}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.features) === characterLimits.features ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.features}/{characterLimits.features}
                  </p>
                </div>
                {errors.features && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Textarea
                    ...
                    placeholder="Describe the main benefits of your product..."
                    className={`resize-none h-32 ${errors.benefits ? 'border-red-500' : ''}`}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.benefits) === characterLimits.benefits ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.benefits}/{characterLimits.benefits}
                  </p>
                </div>
                {errors.benefits && (
                  <p className="mt-1 text-sm text-red-600 flex items-center">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Textarea
                    ...
                    className={`resize-none h-24 ${errors.targetAudience ? 'border-red-500' : ''}`}
                  />
                  <p className={`absolute right-0 -bottom-5 text-xs ${parseInt(characterCounts.targetAudience) === characterLimits.targetAudience ? 'text-orange-500' : 'text-gray-500'}`}>
                    {characterCounts.targetAudience}/{characterLimits.targetAudience}
                  </p>
                </div>
        ...
        <Card className="w-full md:w-1/2">
          ...
          <CardContent>
            {generatedDescription ? (
              ...
              <p className="text-gray-500 italic">Your generated product description will appear here.</p>
            )}
          </CardContent>
        </Card>
      </div>
